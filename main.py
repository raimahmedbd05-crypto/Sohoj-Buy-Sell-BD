...
def handle_all_messages(message):
    # A list of all valid command texts
    valid_commands = ["📤 Gmail Sell", "📥 Gmail Buy", "💳 Balance", "💵 Withdraw",
                      "🌐 Paid VPN Buy", "🎥 YouTube Premium", "👥 Refer", "🆘 Support",
                      "🎁 Play Point Park On", "🇺🇸 USA", "🇹🇼 Taiwan", "🇬🇧 UK", "🇰🇷 South Korean",
                      "📲 Bkash", "📲 Nagad", "↩️ মেনুতে ফিরে যান"]

    if message.text in valid_commands:
        bot.clear_step_handler(message)
        # Manually route to the correct handler
        if message.text == "📤 Gmail Sell":
            gmail_sell(message)
        elif message.text == "📥 Gmail Buy":
            gmail_buy(message)
        elif message.text == "💳 Balance":
            check_balance(message)
        elif message.text == "💵 Withdraw":
            withdraw(message)
        elif message.text == "🌐 Paid VPN Buy":
            vpn_buy(message)
        elif message.text == "🎥 YouTube Premium":
            yt_premium(message)
        elif message.text == "👥 Refer":
            refer(message)
        elif message.text == "🆘 Support":
            support(message)
        elif message.text == "🎁 Play Point Park On":
            play_point_menu(message)
        elif message.text == "↩️ মেনুতে ফিরে যান":
            back_to_home(message)
        elif message.text in ["🇺🇸 USA", "🇹🇼 Taiwan", "🇬🇧 UK", "🇰🇷 South Korean"]:
            process_play_point_country(message)
        elif message.text in ["📲 Bkash", "📲 Nagad"]:
            # Need to know the context to handle payment, so just return to home menu.
            home_menu(message.chat.id)
        return

    # If it's not a valid command, clear the step handler and show an unknown message.
    bot.clear_step_handler(message)
