...
def back_to_home(message):
    bot.clear_step_handler(message.from_user.id)
    home_menu(message.chat.id)
...
def send_broadcast(message):
    ...
    # Check if the message is a photo with a caption or just text
    if message.photo:
...
def handle_all_messages(message):
    ...
    if message.text not in valid_commands and not message.text.startswith('/'):
        unknown_msg = """
❌ অজানা কমান্ড!
